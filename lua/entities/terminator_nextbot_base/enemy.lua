...
function ENT:ShouldBeEnemy(ent)
	...
	if !ent.TerminatorNextBot and ent:IsNPC() and (ent:GetNPCState()==NPC_STATE_DEAD or ent:GetClass()=="npc_barnacle" and ent:GetInternalVariable("m_takedamage")==0) then return false end
	if (ent.TerminatorNextBot or !ent:IsNPC()) and ent:Health()<=0 then return false end
