    ...
    self.TaskList = {
        ...
        ["movement_handler"] = {
            ...
            BehaveUpdateMotion = function( self, data )
                ...
                elseif myTbl.TimeSinceEnemySpotted( self, myTbl ) < 15 and ( not enemyLastPos or entMeta.GetPos( self ):Distance( enemyLastPos ) > 250 ) then
                    myTbl.TaskComplete( self, "movement_handler" )
        ...
        ["movement_shootfromcover"] = {
            ...
            OnDamaged = function( self, data ) -- break out of our trance!
                local myTbl = data.myTbl
                if myTbl and myTbl.NothingOrBreakableBetweenEnemy and myTbl.DistToEnemy < myTbl.GetRealDuelEnemyDist( self, myTbl ) * 0.15 and myTbl.getLostHealth( self ) >= 5 then
            ...
            BehaveUpdateMotion = function( self, data )
                ...
                local needReloadingCovertypes = myTbl.IsReloadingWeapon
                ...
                if needReloadingCovertypes then
                    ...
                    -- force a reload if we have a good chance
                ...
                local exposedMoving
                if hasBrains and feelingExposed and clearOrBreakable and ( scaryEnemy or ( enemysBearingToMe and enemysBearingToMe < 8 ) or lostHealth > 5 ) then
                    exposedMoving = true
                    minCoverType = COVER_SOFTCOVER
                    maxCoverType = COVER_HARDCOVER
                ...
                local needsNewPathGoal = not currPathGoal or badGoalsCoverType or data.NeedsToChangePositions
                ...
                if hasBrains then -- smart ones will avoid going too close
                    hardMinRadius = math.min( duelEnemyDist * 0.25, hardMaxRadius * 0.5 )

                    if scaryEnemy then
                        hardMinRadius = hardMinRadius + 250
                        hardMaxRadius = hardMaxRadius + 1500
                ...
                if needsNewCoverPos and timeForANewCoverpos and data.NextGoalGet < CurTime() then
                    ...
                    local scoreFunction = function( scoreData, area1, area2 )
                        ...
                        local score = scoreData.decreasingScores[area1:GetID()] or 1000
                        ...
                        local outsideDonut
                        if currsDistToEnemy < scoreData.hardMinRadius or currsDistToEnemy > scoreData.hardMaxRadius then
                            if coverFound then
                                score = score / 10
                        ...
                        if not outsideDonut and score >= bestCoverScore then
                            ...
                            for _, coverPosToCheck in ipairs( toCheck ) do
                                if coverPosToCheck:Distance( enemysShoot ) < scoreData.hardMinRadius then continue end
                    ...
                    local fallbackCenter = myTbl.findValidNavResult( self, scoreData, myPos, scoreData.searchRadius, scoreFunction )

                    data.NeedsToChangePositions = nil
                    if coverFound then
                        --print( "good cover", myTbl.GetCoverStatusOfPos( self, myTbl, coverFound, enemy, enemysShoot ) )
                        --debugoverlay.Cross( coverFound, 5, 1, Color( 255, 0, 0 ), true )
                        data.CurrentTaskGoalPos = coverFound
                        data.GoalPosFailures = 0
                        if isFallbackCover then
                            data.AllowFallbackCover = CurTime() + 1

                        end
                    elseif fallbackCenter then
                        data.CurrentTaskGoalPos = fallbackCenter
                        data.AllowFallbackCover = CurTime() + math.random( 5, 10 )
                        data.GoalPosFailures = 0
                ...
                --elseif currPathGoal then
                    --debugoverlay.Cross( currPathGoal, 1, 1, Color( 0, 255, 0 ), true )
                ...
                if needsNewPath then
                    ...
                    if IsValid( currArea ) then
                        ...
                        local oldCount = data.OverusedAreaIds[currAreaId] or 1
                        local weightAdded = 0.25
                        if hasBrains then
                            weightAdded = 1

                        end
                        data.OverusedAreaIds[currAreaId] = oldCount + weightAdded
        ...
        ["movement_backthehellup"] = {
            OnStart = function( self, data )
                ...
                data.myTbl.InvalidatePath( self, "we gotta back the hell up" )
            ...
            BehaveUpdateMotion = function( self, data )
                ...
                elseif distToEnemy > duelEnemyDist * 0.5 then
                    self:TaskComplete( "movement_backthehellup" )
        ...
        ["movement_rushsmartandshoot"] = {
            ...
            BehaveUpdateMotion = function( self, data )
                ...
                if needsNewPathGoal and enemyIsReachable then
                    local furthestGoal
                ...
                elseif result == true then
                    ...
                    elseif goodEnemy and myTbl.DistToEnemy < myTbl.GetRealDuelEnemyDist( self, myTbl ) then
                        self:TaskComplete( "movement_rushsmartandshoot" )
                        myTbl.StartTask( self, "movement_shootfromcover", "i rushed them, time to back off for a bit" )

                    end
                elseif goodEnemy and myTbl.HasBrains and ( scaryEnemy or myTbl.DistToEnemy < wepRange * 0.25 ) then
                    self:TaskComplete( "movement_rushsmartandshoot" )
                    myTbl.StartTask( self, "movement_backthehellup", "im way too close to em" )

                elseif not goodEnemy and sinceLastSpotted > 6 then
                    self:TaskComplete( "movement_rushsmartandshoot" )
        ...
        ["movement_intercept"] = {
            ...
            BehaveUpdateMotion = function( self, data )
                ...
                if needNewGoal and canDoNewGoal then
                    ...
                    self.LastEnemyShootPos = posOnNav.pos
        ...
        ["movement_fanout"] = { -- spread out in straight paths to unoccupied areas, after we get to a spot with no enemy
            OnStart = function( self, data )
                ...
                data.NextGoalGet = 0
                local addTime = math.Rand( 0, 0.5 )
                if not data.myTbl.HasBrains then
                    addTime = addTime * 4 -- stagger stupid bots more, so the pathfinding doesn't all happen at once
        ...
        ["movement_patrol"] = { -- wander the map, walking around, looking for enemies.
            ...
            BehaveUpdateMotion = function( self, data )
                ...
                if data.NextGoalGet < cur then
                    data.NextGoalGet = cur + math.Rand( 0.1, 0.25 )
                    if shouldFollowLeader then
                    ...
                    if not data.CurrentTaskGoalPos and shouldWanderOff and IsValid( data.WatchFromArea ) then
                        ...
                        if not data.WanderingOffStarePos or data.NotSeeCount > data.NotSeeToLookAround then
                            local myShootPos = myTbl.GetShootPos( self )
                            local didOne
                            for _ = 1, 5 do
                            ...
                            if not didOne then
                                ...
                                local prod = terminator_Extras.dirToPos( myShootPos, self.EnemyLastPos ):Dot( terminator_Extras.dirToPos( myShootPos, data.WanderingOffStarePos ) )
                                if prod > 0.75 then -- look for longer if the enemy was this direction
                                    data.NotSeeToLookAround = data.NotSeeToLookAround + math.random( 75, 125 )

                                elseif prod > 0.55 then
                                    data.NotSeeToLookAround = data.NotSeeToLookAround + math.random( 50, 75 )

                                else
                                    data.NotSeeToLookAround = data.NotSeeToLookAround + math.random( 45, 60 )
                ...
                if needsNewPath and data.NextNewPath < CurTime() then
                    ...
                    if IsValid( myArea ) then
                        data.AlreadyPatrolledAreas[ myArea:GetID() ] = true
