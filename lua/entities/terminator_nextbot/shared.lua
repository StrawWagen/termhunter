...
ENT.CloseEnemyDistance = 175 -- bot ignores enemy priority if enemy is this close
...
elseif SERVER then
    ...
    -- optional wraith cloaking logic, only activates if .IsWraith is true
    include( "wraithcloaking.lua" )
...
function ENT:EnemyAcquired( currentTask )
    ...
    local behindEnemy = bearingToMeAbs > 90
    local onEnemysScreen = bearingToMeAbs < 55
    ...
    local beginFirstWatch = self.watchCount == 0 and not reallyAngry and enemy and not enemy.terminator_endFirstWatch and not tooDangerousToApproach
    local stillInFirstWatch = enemy and ( enemy.terminator_endFirstWatch or 0 ) > CurTime() and not reallyAngry and not tooDangerousToApproach
    local doNormalWatch = veryHighHealth and enemyIsNotClose and not self.boredOfWatching and not reallyAngry and not tooDangerousToApproach
    ...
    elseif tooDangerousToApproach then
        local realDuelDist = self:GetRealDuelEnemyDist( entMeta.GetTable( self ) )
        if behindEnemy and math.random( 0, 100 ) < 90 then
            self:TaskComplete( currentTask )
            self:StartTask( "movement_followenemy", nil, "ea, they're very dangerous, but im behind them..." )

        elseif distToEnemySqr < realDuelDist^2 then
            self:TaskComplete( currentTask )
            self:StartTask( "movement_backthehellup", nil, "ea, we gotta back UP!" )

        elseif distToEnemySqr > realDuelDist^2 then
            if onEnemysScreen then
                self:GetTheBestWeapon()
                self:TaskComplete( currentTask )
                self:StartTask( "movement_stalkenemy", nil, "ea, uhhhhh, that's scary, im gonna go somewhere it cant see" )

            else
                self:TaskComplete( currentTask )
                self:StartTask( "movement_camp", nil, "ea, uhhhhh, that's scary, im just gonna... stay here" )
...
function ENT:Initialize()
    ...
    if isnumber( myTbl.HasBrains ) then -- can be bool or num for random chance
...
function ENT:DoDefaultTasks()
    self.TaskList = {
            ...
            BehaveUpdatePriority = function( self, data )
                ...
                if ( not data.UpdateEnemies ) or ( cur > data.UpdateEnemies ) or ( not fodder and data.HasEnemy and not IsValid( prevEnemy ) ) then
                    ...
                    elseif not fodder and not validPotentialNew and not myTbl.IgnoringPlayers( self ) then -- cheap infinite view distance
                        ...
                        coroutine_yield()
                        local alive = pickedPlayer and entMeta.Health( pickedPlayer ) > 0
                ...
                if IsValid( newEnemy ) then
                    ...
                    if isSeeEnemy then
                        ...
                        if not hitNothing and myTbl.DontShootThroughProps then
                            ...
                            nothinOrBreakable = nothinOrBreakable or not shotTr.Hit
        ...
        -- look at enemies, barrels next to them, and shoot!
        -- manage TERM_FISTS state
        ["shooting_handler"] = {
            StartsOnInitialize = true,
            StopsWhenPlayerControlled = true,
            BehaveUpdatePriority = function( self, data, interval )
                ...
                elseif IsValid( enemy ) and not ( myTbl.blockAimingAtEnemy and myTbl.blockAimingAtEnemy > CurTime() ) then
                    ...
                    if wep and myTbl.IsRangedWeapon( self, wep ) then
                        ...
                        if wep.terminatorAimingFunc then
                        ...
                        elseif shootableVolatile and IsValid( shootableVolatile ) then
                            ...
                            local shootAtPos = myTbl.LastEnemyShootPos
                            if myTbl.DistToEnemy < myTbl.CloseEnemyDistance then
                                shootAtPos = myTbl.EntShootPos( self, enemy )

                            end
                            myTbl.shootAt( self, shootAtPos, doShootingPrevent )
        ...
        -- it's really stuck!!!!!!!
        -- background stuck check
        -- tracks bot's position, if it's under a displacement
        -- resets all motion tasks if necessary!
        ["reallystuck_handler"] = {
            StartsOnInitialize = true,
            OnStart = function( self, data )
        ...
        -- tell our buddies on the same .isTerminatorHunterChummy "team", in a radius of .InformRadius, about our shared enemies!
        ["inform_handler"] = {
            StartsOnInitialize = true,
            OnStart = function( self, data )
        ...
        ["movement_backthehellup"] = {
            OnStart = function( self, data )
                ...
                data.DistToQuit = data.DistToQuit or math.max( duelEnemyDist, distToEnemy + duelEnemyDist * 0.5 )
                ...
                data.StartingEnemy = nil
                data.StartingEnemyHealth = 0

                local enemy = myTbl.GetEnemy( self )
                if not IsValid( enemy ) then return end

                data.StartingEnemy = enemy
                data.StartingEnemyHealth = enemy:Health()
            ...
            BehaveUpdateMotion = function( self, data )
                ...
                local currEnemysHealth

                if IsValid( enemy ) then
                    currEnemysHealth = entMeta.Health( enemy )
                    goodEnemy = currEnemysHealth > 0
                    ...
                    awayFromScaryEnemy = ( currEnemysHealth < data.StartingEnemyHealth / 2 )
                    if awayFromScaryEnemy then
                        awayFromScaryEnemy = not IsValid( data.StartingEnemy ) or myPos:Distance( data.StartingEnemy:GetPos() ) > data.DistToQuit
                ...
                elseif awayFromScaryEnemy then
                    self:TaskComplete( "movement_backthehellup" )
                    myTbl.StartTask( self, data.AfterwardsTask, data.AfterwardsTaskData, "current enemy has less than half health of the starting enemy" )
        ...
        -- like movement_backthehellup, but backs away from data.BackupFromPos instead of the current enemy
        -- caller supplies data.BackupFromPos ( vector ), and usually data.DistToQuit
        ["movement_backthehellup_frompos"] = {
            ...
            BehaveUpdateMotion = function( self, data )
                local myTbl  = entMeta.GetTable( self )
                local backupFrom = data.BackupFromPos
                if not backupFrom then
                    ErrorNoHalt( "movement_backthehellup_frompos: data.BackupFromPos is nil\n" )
                ...
                if myTbl.IsSeeEnemy and data.NormalBackupIfEnemy then
                    self:TaskComplete( "movement_backthehellup_frompos" )
                    myTbl.StartTask( self, "movement_backthehellup", {
                        DistToQuit = data.DistToQuit,
                        AfterwardsTask = data.AfterwardsTask,
                        AfterwardsTaskData = data.AfterwardsTaskData,
                    }, "i can see the enemy, back up from them instead" )
                ...
                local needsNewBackupPos = not data.CurrentTaskGoalPos or myPos:Distance( data.CurrentTaskGoalPos ) < 25
                if needsNewBackupPos then
                    local areasToCheck = myNav:GetAdjacentAreas()
                    local areasAlreadyAdded = { myNav = true }
                    for _, area in ipairs( areasToCheck ) do
                        areasAlreadyAdded[area] = true
                    ...
                    for _, area in ipairs( finalAreasToCheck ) do
                        ...
                        local dropoff = myNav:ComputeAdjacentConnectionHeightChange( area )
                        local dirToThere = terminator_Extras.dirToPos( myShoot, myShootWhenImThere )
                        local distWeGet = util.DistanceToLine( myShoot + dirToThere * 10, myShootWhenImThere, backupFrom )
                        distWeGet = distWeGet / dropoff

                        if not terminator_Extras.PosCanSeeComplex( myShootWhenImThere, backupFrom, self ) then -- try and break LOS
                            distWeGet = distWeGet * 2

                        end

                        local beenThereBeforeCount = backedUpCounts[area]
                        if beenThereBeforeCount then
                            distWeGet = distWeGet / beenThereBeforeCount

                        end

                        if not bestBackupDist or distWeGet > bestBackupDist then
                            bestBackupArea = area
                            bestBackupDist = distWeGet
                            bestBackupPos = backupPos
                    ...
                    if not bestBackupPos then
                        if not data.UseNearestPoints then
                            ...
                            myTbl.TaskFail( self, "movement_backthehellup_frompos" )
                            myTbl.StartTask( self, data.AfterwardsTask, data.AfterwardsTaskData, "i can't find a good backup position!" )
                            return
                ...
                -- if we have a holstered weapon, consider pulling it out!
                local canWep, potentialWep = self:canGetWeapon()
                if canWep and self:IsHolsteredWeap( potentialWep ) and self:getTheWeapon( "movement_backthehellup_frompos", potentialWep ) then
                    return
                ...
                if myShoot:Distance( backupFrom ) > data.DistToQuit then
                    self:TaskComplete( "movement_backthehellup_frompos" )
                    myTbl.StartTask( self, data.AfterwardsTask, data.AfterwardsTaskData, "i got far enough from the position!" )
        ...
        -- try and understand an unknown entity by beating it up
        ...
        ["movement_search"] = {
            OnStart = function( self, data )
                data.searchRadius = data.searchRadius or 3000
            ...
            BehaveUpdateMotion = function( self, data )
                ...
                if not hidingToCheck then
                    ...
                    local scoreFunction = function( scoreData, area1, area2 )
                        ...
                        if not localSelf.SearchCheckedNavs[area2sId] then
                            ...
                            for _, spot in ipairs( hidingSpots ) do
                                ...
                                local distToOld
                                if hidingToCheck then
                                    distToOld = hidingToCheck:Distance( scoreData.searchCenter )

                                end
                                if not hidingToCheck then
                                    hidingToCheck = spotOffsetted
                    ...
                    if not hidingToCheck or not IsValid( checkNav ) then -- couldnt find anywhere to check
                        if needsToDoANearbySearch then
                        ...
                        data.hidingToCheck = hidingToCheck
        ...
        -- run off like a headless chicken, searching spots we come across along the way
        ...
        -- stare deeply into the soul of our enemy
        ...
        ["movement_watch"] = {
            ...
            BehaveUpdateMotion = function( self, data )
                local enemy = self:GetEnemy()
                local enemyPos = self:GetLastEnemyPosition( enemy )
        ...
        -- stalk the enemy, keep a wide berth between us and them
        -- basically orbits them, and goes in for the kill if we end up behind them.
        ["movement_stalkenemy"] = {
            OnStart = function( self, data )
                --print( "stalkstart" )
            ...
            BehaveUpdateMotion = function( self, data )
                if not ( data.InvalidAfterwards or data.stalkPos ) then
                    local myPos = self:GetPos()
                    local enemy = self:GetEnemy()
                    local tooDangerousToApproach = self:EnemyIsLethalInMelee( enemy )
                    ...
                    if enemyPos then
                        ...
                        local scoreFunction = function( scoreData, area1, area2 )
                            ...
                            if breakingHardRing and bigTooClose then
                            ...
                            elseif breakingHardRing and bigTooFar then
                                score = score^0.2
                        ...
                        if data.stalkPos then
                            ...
                            if enemyOnNav then
                                ...
                                self:SetupFlankingPath( data.stalkPos, result.area, self.DistToEnemy * 0.8 )

                            else
                                self:SetupPathShell( data.stalkPos )
        ...
        -- path towards the enemy, without taking the shortest route
        ["movement_flankenemy"] = {
            BehaveUpdateMotion = function( self, data )
        ...
        -- basically the bot going "oh crap, did i leave the stove on?"
        ...
        -- path directly towards the enemy
        ["movement_followenemy"] = {
            OnStart = function( self, data )
                ...
                data.tryCreepingUpToEnemy = self:EnemyIsLethalInMelee()
            ...
            BehaveUpdateMotion = function( self, data )
                ...
                if data.baitcrouching or data.tryCreepingUpToEnemy then
                    distToExit = 200
                ...
                if canWep and not GoodEnemy and self:getTheWeapon( "movement_followenemy", potentialWep, "movement_followenemy" ) then
                    return
                elseif self:enemyBearingToMeAbs() < 90 and ( ( self:inSeriousDanger() and GoodEnemy and not ( self:IsReallyAngry() or self:Health() < self:GetMaxHealth() * 0.5 ) ) or self:EnemyIsLethalInMelee( enemy ) ) then
            ...
            ShouldRun = function( self, data )
                if data.tryCreepingUpToEnemy then return false end
                return self:canDoRun()
            end,
            ShouldCrouch = function( self, data )
                if data.tryCreepingUpToEnemy then return true end
            end,
            ShouldWalk = function( self, data )
                local creeping = data.tryCreepingUpToEnemy
                if creeping and self:EnemyIsLethalInMelee() and self:enemyBearingToMeAbs() > 90 then return true end -- creep up to them...
                data.tryCreepingUpToEnemy = nil
                return self:shouldDoWalk()
            end,
        },
        -- shoot the enemy, or punch them to death
        ["movement_duelenemy_near"] = {
            OnStart = function( self, data )
                ...
                data.askedForNewWepSearch = false
                data.killCount = 0
            ...
            OnKillEnemy = function( self, data )
                data.killCount = data.killCount + 1
                data.lastEnemyKill = CurTime()

                if data.askedForNewWepSearch then self:NextWeapSearch( -1 ) return end
                self:ResetWeaponSearchTimers()
                data.askedForNewWepSearch = true
            ...
            BehaveUpdateMotion = function( self, data )
                ...
                if IsValid( enemy ) then
                    scaryEnemy = self:EnemyIsLethalInMelee( enemy ) or self:inSeriousDanger()
                    ...
                    canCover = self:IsRangedWeapon( wep ) and ( scaryEnemy or lowHealth ) and data.failedToTakeCover < 2
                ...
                if IsValid( wep ) then
                    ...
                    rangedWep = self:IsRangedWeapon( wep )
                ...
                elseif
                    ...
                    or ( not rangedWep and not self:CanMoveRightUpToEnemy( enemy ) )
                ...
                if badEnemy then
                    ...
                    if data.badEnemyCounts > 6 or data.fightingPlayer then
                        -- find weapons NOW!
                        if self:IsReallyAngry() then
                ...
                elseif IsValid( enemy ) and self.IsSeeEnemy then
                    ...
                    if self.DistToEnemy < 500 and wep and self:IsRangedWeapon( wep ) then
                        local fistiCuffs = self.DistToEnemy < fisticuffsDist or ( data.quitTime < CurTime() and ( math.random( 0, 100 ) < 25 ) )
                        fistiCuffs = fistiCuffs and not ( wepIsReallyGood or self:inSeriousDanger() )
                        fistiCuffs = fistiCuffs and not ( reallyLowHealth and wepIsOk )
                    ...
                    -- ranged weap
                    if not waterFight and canDoNewPath and ( wepIsOk or canCover ) and rangedWep and IsValid( myNavArea ) then
                        ...
                        -- mowing thru a horde?
                        if ( ( data.lastEnemyKill + 4 ) < CurTime() ) and ( data.killCount >= 2 ) and ( not scaryEnemy ) and ( wep:Clip1() > 0 ) then
                            self:InvalidatePath( "horde! im gonna hold my ground and mow them down" )
        ...
        ["movement_inertia"] = {
            ...
            BehaveUpdateMotion = function( self, data )
                if not ( data.InvalidateAfterwards or data.setupPath ) then
        ...
        ["movement_biginertia"] = {
            OnStart = function( self, data )
                ...
                self.bigInertiaPreserveBeenAreas = nil -- gobble it up
        ...
        ["movement_camp"] = {
            ...
            BehaveUpdateMotion = function( self, data )
                ...
                if self.IsSeeEnemy then
                    ...
                    local killerr = enemy and enemy.isTerminatorHunterKiller

                    data.wasEnemy = true
                    data.notSeeCount = 0
                    data.notSeeToLookAround = 0
                    self.totalCampingCount = self.totalCampingCount + 3
                    if enemyBearingToMeAbs < 10 then
                        if killerr then
                            -- nopenope
                    ...
                    elseif enemyBearingToMeAbs > 20 or standingSortaStillAndBored or veryBored then
                ...
                if self.IsSeeEnemy and self.DistToEnemy < tooCloseDist then
                    self:TaskComplete( "movement_camp" )
                    if tooDangerousToApproach then
                        if self.DistToEnemy < self.RunSpeed * 2 and data.lookinRightAtMeCount < 25 and self:enemyBearingToMeAbs() < 90 then
                            self:StartTask( "movement_followenemy", nil, "scary guy is getting close, but they dont see me" )
        ...
        -- place hardcoded "trap" weapons
        -- needs these 2 functions on wep's table ( see 'weapon_slam_term' for example )
        ...
        -- close in on an enemy someone on our .isTerminatorHunterChummy "team" spotted
        -- lots of flanking and checks to make sure bots come in from all angles
        ["movement_intercept"] = {
            OnStart = function( self, data )
                if not self.isUnstucking then
