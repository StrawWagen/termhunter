...
function ENT:findValidNavResult( data, start, radius, scoreFunc, noMoreOptionsMin )
    ...
    while not table_IsEmpty( opened ) do
        ...
        table_insert( closedSequential, areaId )
...
function ENT:Use2( toUse )
    ...
    if not successful then useClassBlacklist[class] = true return end
...
function ENT:tryToOpen( blocker, blockerTrace )
    ...
    if self.IsStupid then
        ...
        elseif math.random( 0, 100 ) < 50 and ( CurTime() - startedTryingToOpen ) > 0.5 then
            self:WeaponPrimaryAttack()
...
local speedToBlocker = 25^2

function ENT:BehaviourThink()
    if self.loco:GetVelocity():LengthSqr() > speedToBlocker and not self:IsControlledByPlayer() and not self:DisableBehaviour() then
...
function ENT:canIntercept( data )
    ...
    local notHeadingToEnd = self.lastInterceptPos and SqrDistGreaterThan( self.lastInterceptPos:DistToSqr( self:GetPath():GetEnd() ), 800 )
    ...
    local intercept = freshIntercept and notHeadingToEnd and not self.IsSeeEnemy
...
function ENT:walkArea()
    ...
    self.nextFloodMarkWalkable = CurTime() + math.Rand( 0.5, 1 )
...
hook.Add( "OnNPCKilled", "terminator_markkillers", function( npc, attacker, inflictor )
    if npc.isTerminatorHunterChummy ~= "terminator" then return end
    ...
    self.TaskList = {
        ["shooting_handler"] = {
            ...
            BehaveUpdate = function( self, data, interval )
                ...
                elseif wep.Clip1 and ( wep:GetMaxClip1() > 0 ) and ( wep:Clip1() < wep:GetMaxClip1() / 2 ) and not self.IsReloadingWeapon then
                    self:WeaponReload()
        ...
        ["reallystuck_handler"] = { -- it's really stuck!!!!!!!
            ...
            BehaveUpdate = function( self, data )
                ...
                if nextCache < CurTime() then
                    ...
                    if not self.IsSeeEnemy and ( stuck or sortaStuck or underDisplacement or overrideStuck ) then -- i have been in the same EXACT spot for S I Z E seconds
                    ...
                    if forceupdateenemies or not data.UpdateEnemies or CurTime() > data.UpdateEnemies or data.HasEnemy and not IsValid( prevenemy ) then
                        ...
                        elseif IsValid( enemy ) then
                            ...
                            if self.IsSeeEnemy and not self.WasSeeEnemy then
                                ...
                                local added = math.Rand( 0.3, 0.6 )
                                if self.DistToEnemy > 2500 then
                                    added = math.Rand( 0.9, 0.15 )
        ...
        ["movement_bashobject"] = {
            ...
            BehaveUpdate = function( self, data, interval )
                ...
                if not IsValid( aBashingFrenzyTerminator ) and #ents.FindByClass( self:GetClass() ) >= 2 then
                    -- global var!!!
        ...
        ["movement_getweapon"] = {
            OnStart = function( self, data )
                ...
                data.crapWep = function()
                    ...
                    if data.Wep.failedWeaponPaths > 15 then
            ...
            BehaveUpdate = function( self, data )
                ...
                if canNewPath then
                    ...
                    if not self:primaryPathIsValid() then
                        ...
                        if self:CanBashLockedDoor( nil, 1000 ) and self:BashLockedDoor( "movement_getweapon" ) then
                            return
        ...
        ["movement_search"] = {
            OnStart = function( self, data )
                ...
                data.searchRadius = data.searchRadius or 2000
                ...
                local scoreFunction = function( scoreData, area1, area2 )
                    ...
                    if not localSelf.SearchCheckedNavs[area2sId] then
                        ...
                        for _, spot in ipairs( hidingSpots ) do
                            ...
                            if not util.IsInWorld( spotOffsetted ) then continue end
                            ...
                            elseif distToOld and distToNew > ( distToOld + 1000 ) then
                                return math.huge
            ...
            BehaveUpdate = function( self, data )
                ...
                if searchWantInternal <= 0 then
                    ...
                    if math.random( 0, 100 ) < 20 and not self:IsMeleeWeapon( myWep ) then
                ...
                elseif doneCount > 2 and self:canIntercept( data ) then
                    self:TaskComplete( "movement_search" )
                    self:StartTask2( "movement_intercept", nil, "i can intercept someone" )
                    return
                ...
                elseif isSoundHint and distToSound < soundHintDistNeeded then
        ...
        ["movement_stalkenemy"] = {
            ...
            BehaveUpdate = function( self, data )
                ...
                local scary = ( self:inSeriousDanger() and IsValid( enemy ) and not self:IsReallyAngry() ) or self:EnemyIsLethalInMelee()
                ...
                local ambush = enemyBearingToMeAbs > 90 and IsValid( enemy ) and self.IsSeeEnemy and not exposed and reachable and not scary
                ...
                if canGetWeap and hiddenOrUnholstering and self:getTheWeapon( "movement_stalkenemy", potentialWep, "movement_stalkenemy" ) then
                    ...
                    self.PreventShooting = nil
                    return
                elseif self:canIntercept( data ) and ( self.WasHidden or interceptIsVeryGood ) then
        ...
        ["movement_approachforcedcheckposition"] = {
            ...
            BehaveUpdate = function( self, data )
                ...
                elseif data.Unreachable and givenItAChance then
                    self:TaskFail( "movement_approachforcedcheckposition" )
                    if toPos then
                        self:StartTask2( "movement_perch", { requiredTarget = toPos, perchRadius = self:GetRangeTo( toPos ) * 1.5, distanceWeight = 0.01 }, "i cant reach the pos, ill try looking at it?" )
        ...
        ["movement_duelenemy_near"] = {
            ...
            BehaveUpdate = function( self, data )
                ...
                elseif self:EnemyIsLethalInMelee() or self:inSeriousDanger() and enemy and not data.wasStalk then
        ...
        ["movement_camp"] = {
            ...
            BehaveUpdate = function( self, data )
                ...
                elseif ( not self.IsSeeEnemy and data.notSeeCount > ( data.maxNoSeeing / 8 ) ) or data.lookinRightAtMeCount > 150 then
                    self:TaskComplete( "movement_camp" )
                    self:StartTask2( "movement_perch", { requiredTarget = self.EnemyLastPos, perchRadius = 500, distanceWeight = 1 }, "i lost sight of them" )
                    ...
                    for _ = 1, 10 do
                        ...
                        for _, checkPos in ipairs( checkPositions ) do
                            ...
                            if data.requiredTarget then
                                ...
                                local hitNearby = SqrDistLessThan( canSeeTr.HitPos:DistToSqr( data.requiredTarget ), 250 )
                                local hitAlmost = SqrDistLessThan( canSeeTr.HitPos:DistToSqr( data.requiredTarget ), 35 )
                                if hitAlmost then
                                    canSeeTargetMul = 100

                                elseif hitNearby then
                                    canSeeTargetMul = 8
                            ...
                            if enemysLastPos then
                                local _, canSeeEnemysLastPosTr = terminator_Extras.PosCanSeeComplex( checkPos, enemysLastPos, self )
                                local hitAlmost = SqrDistLessThan( canSeeEnemysLastPosTr.HitPos:DistToSqr( enemysLastPos ), 35 )
                                if hitAlmost then
                                    canSeeTargetMul = canSeeTargetMul * 2
        ...
        ["inform_handler"] = {
            ...
            BehaveUpdate = function( self, data, interval )
                ...
                data.EnemyPosInform = CurTime() +  math.Rand( 3, 5 )
            ...
            InformReceive = function( self, data, enemy, pos, senderpos )
                ...
                local enemIsPly = enemy:IsPlayer()
                -- dont go after npcs when plys are around!
                if self.lastInterceptIsPly and not enemIsPly then return end
                ...
                self.lastInterceptIsPly         = enemIsPly
