...
if SERVER then
	util.AddNetworkString("weapon_ar2_term.muzzleflash")
	util.AddNetworkString("weapon_ar2_term.combineballwhiz")
else
	killicon.AddFont("weapon_ar2_term","HL2MPTypeDeath","2",Color(255,80,0))
end
...
function SWEP:DoMuzzleFlash()
	if SERVER then
		net.Start("weapon_ar2_term.muzzleflash",true)
...
if CLIENT then
	net.Receive("weapon_ar2_term.muzzleflash",function(len)
		...
		for i=1,#clients do
			...
			if delta:Dot(vel)>0.5 then
				...
				if dist<200 then
					...
					net.Start("weapon_ar2_term.combineballwhiz",true)
						net.WriteEntity(self)
					net.Send(ply)
				end
			end
...
if CLIENT then
	net.Receive("weapon_ar2_term.combineballwhiz",function(len)
		local ent = net.ReadEntity()
